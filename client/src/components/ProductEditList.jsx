...
export default function ProductEditList({products,onUpdated}){
    ...
    const deleting=async id=>{if(confirm('Delete this product?')){await api.delete(`/products/${id}`);onUpdated();}};
    ...
    return(
        <div>
            ...
            {pEdit&&<div className="mt-6"><ProductForm product={pEdit} onSuccess={()=>{onUpdated();setEditId(null);}}/></div>}
        </div>
    );
}
                ...
                <div className="hidden md:flex items-center space-x-4">
                    {categories.map(cat => (
                        <div key={cat.name} className="relative">
                            <button
                                onClick={() => setDesktopOpen(desktopOpen === cat.name ? null : cat.name)}
                                className="px-3 py-2 hover:bg-primary-700 rounded inline-flex items-center focus:outline-none"
                            >
                                <span>{cat.name}</span>
                            ...
                            {cat.options.length > 0 && desktopOpen === cat.name && (
                ...
                <div className="md:hidden bg-primary-700 px-4 pb-4 space-y-2">
                    {categories.map(cat => (
                        <div key={cat.name}>
                            <button
                                onClick={() =>
                                    setDesktopOpen(desktopOpen === cat.name ? null : cat.name)
                                }
                            ...
                            {desktopOpen === cat.name && cat.options.length > 0 && (
